...
//   const sections = ['Home', 'How it Works', 'Reviews', 'About']; 

//     // Scroll listener for detecting active section
//   useEffect(() => {
//     const handleScroll = () => {
...
//   return (
//     <nav className="fixed w-full z-50 mx-auto py-4 px-4 backdrop-filter bg-opacity-30 bg-white font-work-sans text-black">
...
const Navbar = () => {
  const [isOpen, setIsOpen] = useState(false);
  const [activeSection, setActiveSection] = useState("home");
  const sections = ["Home", "How it Works", "Reviews", "About"]; // Nav Items

  // Scroll listener for detecting active section
  useEffect(() => {
    const handleScroll = () => {
      const sections = document.querySelectorAll("div[id]"); // Select divs with IDs
      let current = "";
      ...
      setActiveSection(current); // Update active section
    };
        ...
        <div className="hidden md:flex flex-1 justify-center items-center space-x-8">
          {sections.map((item) => (
            <a
              ...
              onClick={(e) =>
                handleLinkClick(e, item.toLowerCase().replace(/\s+/g, "-"))
              }
            >
              {item}
            </a>
